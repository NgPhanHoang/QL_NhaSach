...
namespace QUANLYNHASACH
{
    public partial class ThanhToan : Form
    {
        ...
        private void ThanhToan_Load(object sender, EventArgs e)
        {
            ...
            loadtg_cbb();
            loadtl_cbb();
            label5.Text = "Người Nhập Hóa đơn: " + User.Username;
        ...
        {
            txt_masach.Enabled = txt_tensach.Enabled = cbb_tacgia.Enabled = cbb_theloai.Enabled = txt_dongia.Enabled = true;
        ...
        private void btn_themsach_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (slconlai >= slban) // Sửa điều kiện so sánh để cho phép số lượng trong kho bằng số lượng bán
                {
                    float dongia = float.Parse(txt_dongia.Text);
                    int sl = int.Parse(txt_soluong.Text);
                    float tongtien = dongia * sl;
                    int mahdx = int.Parse(txt_mahoadonxuat.Text);

                    string insertString = "INSERT INTO CTHOADONXUAT (MAHDX, MASACH, SOLUONG, DONGIA, TONGTIEN, TAIKHOAN) VALUES (@mahdx, @masach, @soluong, @dongia, @tongtien, @taikhoan)";
                    SqlCommand cmd = new SqlCommand(insertString, connsql);
                    ...
                    cmd.Parameters.AddWithValue("@taikhoan", User.Username);
                    cmd.ExecuteNonQuery();
                    ...
                    string updateSach = "UPDATE SACH SET SOLUONG = SOLUONG - @soluong WHERE MASACH = @masach";
                    SqlCommand cmd1 = new SqlCommand(updateSach, connsql);
                    cmd1.Parameters.AddWithValue("@soluong", sl);
                    cmd1.Parameters.AddWithValue("@masach", txt_masach.Text);
        ...
        private void btn_thanhtoan_Click(object sender, EventArgs e)
        {
            ...
            tong = tong + tam;
            string nguoiThemHoaDon = User.Username;
            string khachhang = cbb_kh.Text;
            string tenSach = txt_tensach.Text;
            string  soLuong = txt_soluong.Text;
            MessageBox.Show(
                "Tổng tiền phải thanh toán là: " + tong.ToString() +
                "\nNgười thêm hóa đơn: " + nguoiThemHoaDon +
                "\nKhách hàng: " + khachhang.ToString(),
                "Hóa Đơn",
        ...
        private void btn_inhdxuat_Click(object sender, EventArgs e)
        {
            ...
            string strselect = "select * from hoadonxuat where MAHDX= "+txt_mahoadonxuat.Text+"";
            SqlDataAdapter da = new SqlDataAdapter(strselect, connsql);
            da.Fill(dt);
        ...
        private void rdb_tacgia_CheckedChanged(object sender, EventArgs e)
        {
